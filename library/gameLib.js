...
function setGame(res = ""){
  ...
  game.font = "Calibri-normal-20pt-left-putih-normal-1.6";		
...
function tombolditekan(event) {
    ...
    if (!game.pause && game.aktif){
      ...
      if (event.keyCode == 32) {atas = true; game.atas = true;}
      ...
      if (event.keyCode == 13) {enter = true; game.enter = true;}
      if (event.keyCode == 27) {esc = true; game.esc = true;}
    }
    if (event.keyCode == 80) {
      tombolP = true;					
    }
...
function tomboldilepas(event) {
    ...
    if (event.keyCode == 32) {atas = false; game.atas = false;}
    ...
    if (event.keyCode == 13) {enter = false; game.enter = false;}
    if (event.keyCode == 27) {esc = false; game.esc = false;}
    if (event.keyCode == 80) {
      tombolP = false;
...
function playSound(id, volume) {
  const sound = gameSound(id, volume);
	sound.play();
}

function stopSound(param, id) {
  if (param) {
    const soundElement = document.getElementById(id);
    soundElement.pause();
    param = false;
...
function latar(img) {
  konten.drawImage(img, 0, 0, screenW, screenH);
}
